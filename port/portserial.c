...
xMBPortSerialInit( UCHAR ucPORT, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity )
{
  ...
  if(ucDataBits==8)
  {
    UART2->CR1 &= (uint8_t)(~UART2_CR1_M); // 8 data bits 
  }
  else
  {
    UART2->CR1 |= (uint8_t)(UART2_CR1_M); // 9 data bits
  }
  ...
  */
  ...
  UART2->CR3 |= (uint8_t)0x00;  //  UART1_STOPBITS_1
...
//  UART1->CR1 |= (uint8_t)Parity;
  ...
  {
  case MB_PAR_NONE:
   UART2->CR1 |= (uint8_t)(0x00);
   break;
  case MB_PAR_ODD:
   UART2->CR1 |= (uint8_t)(0x06);
   break;
  case MB_PAR_EVEN:
   UART2->CR1 |= (uint8_t)(0x04);
   break;
  default:
   break;
  }
  ...
  UART2->CR3 |= (uint8_t)((uint8_t)0x80 & (uint8_t)(UART2_CR3_CPOL | 
